...
def create_queries(prompt):
    query_msg = (
        ...
        'you have ever had with the user. Wtih first principles create a Python list of queries to '
        'search the embedding database for any data that would be necessary to have access to in '
        'order to correctly respond to the prompt. Your response must be a Python list with no syntaxt errors. '
        'Do not explain anything and do not ever generate anything but a perfect syntax Python list.'
    )
    query_convo = [
        {'role': 'system', 'content': query_msg},
        {'role': 'user', 'content': 'Write an email to my car insurrance company and create a pursuasive request for them to lower my monthly rate.'},
        {'role': 'assistant', 'content': '["What is the user name?", "What the users current auto insurance provider?", "What is the monthly rate the usser currently pays for auto isurance?"]'},
...
def classify_embedding(query, context):
    classify_msg = (
        ...
        'If the context is seeemingly exaclty what the search query needs, respond "yes" if it is anything but directly '
        'related respond "no". Do not respond "yes" unless the content is highly relevant to the search query.'
    )    
    classify_convo = [
        {'role': 'system', 'content': classify_msg},
        {'role': 'user', 'content': f'SEARCH QUERY: What is the users name? \n\nEMBEDDED CONTEXT: You are Patrick Chouinard. How can I help today Patrick?'},
...
conversations = fetch_converations()
